...
(defun history-apply (history segs)
  (mapcar (lambda (seg)
            (let* ((pron (senn-kkc:segment-pron seg))
              ...
              (if (not history-form)
                  ...
                  (senn-kkc:make-segment
                   :pron pron
                   :candidates
                   (cons (senn-kkc:make-candidate :form history-form)
                         (remove history-form
                                 (senn-kkc:segment-candidates seg)
                                 :key #'senn-kkc:candidate-form
                                 :test #'string=))))))
          segs))
...
(defmethod senn-kkc:convert ((kkc history-overwrite-mixin) (pron string)
