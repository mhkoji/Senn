...
HRESULT __stdcall CandidateWindow::QueryInterface(REFIID riid, void ** ppvObject)
{
...
ULONG __stdcall CandidateWindow::AddRef(void)
{
    return ++ref_count_;
}
...
{ 0x68cc0134, 0x855f, 0x4216, { 0xa6, 0xeb, 0x7d, 0x56, 0x8b, 0xb8, 0x8, 0xc2 } };
...
{ 
  ...
  if (thread_mgr_->QueryInterface(IID_ITfUIElementMgr, (void**)&ui_mgr) == S_OK)
  {
...
CandidateWindow* CandidateWindow::Create(ITfContext *context, ITfThreadMgr *thread_mgr) {
  ...
  ITfUIElementMgr *ui_mgr = nullptr;
  if (thread_mgr->QueryInterface(IID_ITfUIElementMgr, (void**)&ui_mgr) == S_OK)
  {
    if (ui_mgr != nullptr)
    {
...
void CandidateWindow::UnregisterWindowClass() {
  UnregisterClassW(senn::senn_win::kSennCandidateWindowClassName, g_module_handle);
}
