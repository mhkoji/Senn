    ...
    (let ((cycle nil)
      ...
      (if (null cycle)
          ...
          (let* ((cycle-vertices (alexandria:hash-table-keys cycle))
            ...
            ;; 縮約したグラフで再帰呼び出し
            (let ((edges (chu-liu-edmonds num-vertices
                                          score-hash
                                          current-vertices))
                  (final-edges nil))
              (if (null edges)
...
(defun test1 ()
  (let ((N 4)
    ...
    (setf (get-score s 0 1)  9)
    (setf (get-score s 0 2) 10)
    (setf (get-score s 0 3)  9)
    (setf (get-score s 1 2) 20)
    (setf (get-score s 1 3) 11)
    (setf (get-score s 2 1) 30)
...
(defun test2 ()
  (let ((N 8)
        (s (make-hash-table :test #'equal)))
    (setf (get-score s 0 1)  1)
    (setf (get-score s 0 2)  3)
    (setf (get-score s 0 3)  1)
    (setf (get-score s 1 2)  2)
    (setf (get-score s 1 4)  5)
    (setf (get-score s 2 5) 10)
    (setf (get-score s 3 2)  1)
    (setf (get-score s 3 7)  8)
    (setf (get-score s 4 2) 10)
    (setf (get-score s 5 4) 10)
...
(defun test3 ()
  (let ((N 4)
        (s (make-hash-table :test #'equal)))
    (setf (get-score s 0 1)   3)
    (setf (get-score s 0 2) 1.5)
